...
import com.ucss.elementary.weather.constant.IsNeeded;
...
public class ImportExeclUtil
{
    ...
    private static Object getEntityMemberValue(Object realValue, Field[] fields, int f, String cellValue)
    {
        ...
        switch (type)
        {
            ...
            case "java.lang.Float":
                realValue = StringUtils.isBlank(cellValue) ? null : new Float(cellValue);
                break;
            case "java.lang.Long":
                realValue = StringUtils.isBlank(cellValue) ? null : new Long(cellValue);
                break;
            case "java.lang.Short":
                realValue = StringUtils.isBlank(cellValue) ? null : new Short(cellValue);
                break;
    ...
    public static String makeExcel(InputStream inputstream,List<Map<String,String>> list,String reName,String targetPath) throws Exception {
        ...
        try {
            ...
            //获取列数
            Short colCount=Short.valueOf(String.valueOf(sheet.getRow(0).getLastCellNum()+Short.valueOf("1")));
            //遍历
            for (Map<String,String> item:list) {
                ...
                for (Short i=1;i<=colCount;i++) {
                    ...
                    //赋值
                    if (null != item.get(String.valueOf(i)) && !"".equals(item.get(String.valueOf(i)))){
                        cell.setCellValue(item.get(String.valueOf(i)));
                    }else {
                        cell.setCellValue("");
                    }
                }
         ...
         //将excel表存放在目录下
            File descFile = new File(targetPath+reName+dateTime+".xlsx");
            if (!descFile.getParentFile().exists()) {
                descFile.getParentFile().mkdirs();
            }
            os = new FileOutputStream(targetPath + reName + dateTime + ".xlsx");
            StringBuilder builder = new StringBuilder();
            return  builder.append(targetPath + reName + dateTime + ".xlsx").toString();
        }catch (Exception e){
            e.printStackTrace();
            return null;
        }finally {
